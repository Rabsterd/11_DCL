 ...
 * 
 *  사용자 계정 : 데이터베잉스에 대하여 질의, 갱신, 보고서 작성 등의
 * 					작업을 수행할 수 있는 계정으로
...
CREATE USER gms_sample IDENTIFIED BY 1234;



-- 2. 새 연결(접속방법) 추가
--> gms_sample은 CREATE SESSION 권한을 가지고 있지 않음; 로그온이 거절되었습니다
--> 접속 권한
...
GRANT CREATE SESSION TO gms_sample;
...
CREATE TABLE TB_TEST(
 ...
 GRANT CREATE TABLE TO gms_sample;

ALTER USER gms_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO gms_sample;
...
-- kh / gms_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh.EMPLOYEE;
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) gms_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO gms_sample;
...
REVOKE SELECT ON EMPLOYEE FROM gms_sample;

--5. (sample) 다시 kh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh.EMPLOYEE;
